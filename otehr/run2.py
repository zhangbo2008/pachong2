...
@app.route('/api/fun2', methods=['GET'])
def fun122():
    return "model"


# 这个接口使用方法: postman里面 body---form-data----key里面类型选file--value选文件即可.
@app.route('/api/upload2', methods=['POST', 'GET'])
def upload4422():
...
@app.route('/api/jiexi2', methods=['POST', 'GET'])
def upload222():
    # 下面这一段是通用的.
    result = Auth.identify(Auth, request)
    print(898989898989,result)
...
@app.route('/api/save2', methods=['POST', 'GET'])
def upload45422():
    # 下面这一段是通用的.
    result = Auth.identify(Auth, request)
    print(898989898989,result)
...
from app import db
@app.route('/api/baocun2', methods=['POST', 'GET'])
def upload45466622():
    # 下面这一段是通用的.
    result = Auth.identify(Auth, request)
    print(898989898989,result)
...
from app import db
@app.route('/api/huifu2', methods=['POST', 'GET'])
def upload45466688822():
    # 下面这一段是通用的.
    result = Auth.identify(Auth, request)
    print(898989898989,result)
...
@app.route('/api/chakan2', methods=['POST', 'GET'])
def upload222222():
    # 下面这一段是通用的.
    result = Auth.identify(Auth, request)
    print(898989898989,result)
...
@app.route('/api/liebiao2', methods=['POST', 'GET'])
def upload2222222222():
    # 下面这一段是通用的.
    result = Auth.identify(Auth, request)
    print(898989898989,result)
...
@app.route('/api/delete2', methods=['POST', 'GET'])
def upload222444222223():
    # 下面这一段是通用的.
    result = Auth.identify(Auth, request)
    print(898989898989,result)
