...
class DmozSpider4(scrapy.Spider): # 继承Spider类
    ...
    html='  http://www.xinhuanet.com/world/syxw.htm'
    html=html.strip()
...
# 下面是直接匹配方式获取所有index页. 也就是一般需求这么跑就可以获取所有index页了.
    ...
    def parse(self, response): # 一级爬取代码
        ...
        div_list = response.xpath('//li[@class="clearfix"]//a/@href')  # 加入正则
    ...
    def parse_detail(self, response):  # 二级爬取代码
       ...
       #下面要设计多重xpath判断.因为格式不同意.
        ...
        item['neirong']= response.xpath('//div[@class="m-detail"]//p').extract()
        item['neirong']+= response.xpath('//div[@id="p-detail"]//p').extract()
