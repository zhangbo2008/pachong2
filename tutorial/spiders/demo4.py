...
class DmozSpider4(scrapy.Spider): # 继承Spider类
    ...
    html就是要爬取的网站.
    '''
    name = "dmoz4" # 爬虫的唯一标识，不能重复，启动爬虫的时候要用
    html='http://www.kekenet.com/read/news/'
    ...
    if 0:#调试用, 一般不用这么跑.这个只是动态js代码需要这么使用而已. 一般网页没有这种方式.这个方式太慢爬虫.但是可以避免不必要的js bug
     while 1:
        ...
        if now==None:
            ...
            saveall.append(now)
    tmp=r'http://www.kekenet.com/read/news/List_'
    ppp=[]
    for i in range(1900,1930):
        ppp.append(tmp+str(i)+'.shtml')
    saveall=[html]+ppp
    ...
    def parse(self, response): # 一级爬取代码
        ...
        div_list = response.xpath('//h2/a[contains(@href, "http") and contains(@href,"shtml")]/@href')  # 加入正则

        print([i.extract() for i in div_list],787878787878787878787)
        # print(div_list[0])
        # print(div_list[-1])
        # print((div_list))
        print("进入了一级爬虫")
    ...
    def parse_detail(self, response):  # 二级爬取代码
        ...
        item['neirong']= response.xpath('//div[@id="article"]//text()').extract()
