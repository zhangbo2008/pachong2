...
class DmozSpider4(scrapy.Spider): # 继承Spider类
    ...
    html='  http://www.ecocn.org/portal.php'
    html=html.strip()
...
# 下面是直接匹配方式获取所有index页. 也就是一般需求这么跑就可以获取所有index页了.
         ...
         while 1:
            ...
            for s in bs('a'):
                ...
                if s.text=="下一页►":
                    ...
                    print("loook",now)
                    # 注意这种旧网站的编码方式.
                    now = 'http://www.i21st.cn'+parse.quote(now.get('href'), safe=";/?:@&=+$%, ", encoding="gbk") # 中文的处理方式是里面加上%即可!!!!!!!!!!!!!!!!!!!!!!
    ...
    def parse(self, response): # 一级爬取代码
        ...
        div_list = response.xpath('//div[@class="module cl xl"]/ul/li//a/@href')  # 加入正则
        # div_list = response.xpath('//div[@class="module cl xl"]/ul/li')  # 加入正则

        # print(85654645654, div_list)
        div_list=['http://www.ecocn.org/'+i.extract() for i in div_list]
        print(85654645654, div_list)
        # 去掉调回的情况.
    ...
    def parse_detail(self, response):  # 二级爬取代码
       ...
       #下面要设计多重xpath判断.因为格式不同意.
        ...
        item['neirong']+= response.xpath('//div[@id="storyBody"]//p').extract()
