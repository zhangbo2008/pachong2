...
# 下面是直接匹配方式获取所有index页. 也就是一般需求这么跑就可以获取所有index页了.
         ...
         while 1:
            ...
            for s in bs('a'):
                ...
                if s.text=="»":
                    ...
                    print("loook",now)
                    # 注意这种旧网站的编码方式.
                    now = r'https://'+'jp.hjenglish.com'+parse.quote(now.get('href'), safe=";/?:@&=+$%, ", encoding="gbk") # 中文的处理方式是里面加上%即可!!!!!!!!!!!!!!!!!!!!!!
    ...
    saveall=[
        ...
        #'http://www.171english.cn/news/2018/june/',
             'https://jp.hjenglish.com/new/tag/%E4%B8%AD%E6%97%A5%E5%8F%8C%E8%AF%AD/',
    ...
    def parse(self, response): # 一级爬取代码
        ...
        div_list = response.xpath('//li[@class="article-item"]/a[@class="big-link title-article"]/@href')  # 加入正则


        div_list=['https://jp.hjenglish.com'+i.extract() for i in div_list]
    ...
    def parse_detail(self, response):  # 二级爬取代码
       ...
       #下面要设计多重xpath判断.因为格式不同意.
        ...
        item['neirong']= response.xpath('//div[@class="article-content"]').extract()
