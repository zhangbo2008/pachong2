...
# 下面是直接匹配方式获取所有index页. 也就是一般需求这么跑就可以获取所有index页了.
         ...
         while 1:
            ...
            for s in bs('a'):
                ...
                if s.text=="下一页":
                    ...
                    print("loook",now)
                    # 注意这种旧网站的编码方式.
                    now = r'https://www.tantuw.com/'+parse.quote(now.get('href'), safe=";/?:@&=+$%, ", encoding="gbk") # 中文的处理方式是里面加上%即可!!!!!!!!!!!!!!!!!!!!!!
    ...
    saveall=[
        ...
        #'http://www.171english.cn/news/2018/june/',
             'https://www.tantuw.com/riyu/fengsu',
    ...
    def parse(self, response): # 一级爬取代码
        ...
        div_list = response.xpath('//ul[@class="threelist clearfix"]/li/a/@href')  # 加入正则


        div_list=['https://www.tantuw.com/'+i.extract() for i in div_list]
    ...
    def parse_detail(self, response):  # 二级爬取代码
       ...
       #下面要设计多重xpath判断.因为格式不同意.
        ...
        item['neirong']= response.xpath('//div[@class="clearfix mt10 news_text"]//p').extract()
