...
def get_weights_name(transformer_name, lowercase):
    if transformer_name == 'bert' and lowercase:  #其中 Uncased 在使用 WordPiece 分词之前都转换为小写格式，并剔除所有 Accent Marker，而 Cased 会保留它们。项目作者表示一般使用 Uncased 模型就可以了，除非大小写对于任务很重要才会使用 Cased 版本。
    ...
    if transformer_name == 'xlnet':
        return 'xlnet-base-cased'  # xlnet模型是区分大小写的,所以是cased模型.
...
class GecBERTModel(object):
    def __init__(self, vocab_path=None, model_paths=None,
        ...
        for model_path in model_paths:
            ...
            model = Seq2Labels(vocab=self.vocab,  # 模型核心就是seq2labels这个算法,里面只是简单的linear模型分类问题,所以我们之后可以进行改进!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                               text_field_embedder=self._get_embbeder(weights_name, special_tokens_fix),
                               confidence=self.confidence  # 获取bert编码,单过网络embed层的权重.
                               ).to(self.device)
            if torch.cuda.is_available():
                model.load_state_dict(torch.load(model_path))
            else:
                model.load_state_dict(torch.load(model_path, # 获取model其他部分的权重.
...
#token变回raw
    ...
    def _get_embbeder(self, weigths_name, special_tokens_fix):
        ...
        text_field_embedder = BasicTextFieldEmbedder( # 就是再封装一层.
    ...
    def preprocess(self, token_batch): # raw-------> token
        ...
        for indexer in self.indexers:
            ...
            for sequence in token_batch:
                ...
                tokens = [Token(token) for token in ['$START'] + tokens] # 强行价格start标签在开头
    ...
    def handle_batch(self, full_batch):
        ...
        for n_iter in range(self.iterations): # 迭代5次.
