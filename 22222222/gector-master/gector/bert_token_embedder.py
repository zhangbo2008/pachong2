...
class PretrainedBertModel:
    ...
    def load(cls, model_name: str, cache_model: bool = True) -> PreTrainedModel:
        ...
        model = AutoModel.from_pretrained(model_name) # 利用automode接口直接获取.这一行需要内存很大,docker需要开6个G.
        if cache_model:
            cls._cache[model_name] = model# 就是把model写到类里面.方便调用.
...
class PretrainedBertEmbedder(BertEmbedder): # 这个类用来复用bert预训练数据.
    ...
    def __init__(
        ...
        if special_tokens_fix:
            try:
                vocab_size = self.bert_model.embeddings.word_embeddings.num_embeddings# 看是否有这个属性.
